...
size_t time_sha_avx(){
  ...
  uint8_t** msgs;
  msgs = malloc(sizeof(uint8_t*)*(NMSGS/8));
  for (size_t i = 0; i<(NMSGS/8); ++i) {
    msgs[i] = malloc(64*8);
    getrandom(msgs[i], 8*64, 1);
  }
  ...
  for (size_t i = 0; i<(NMSGS/8); ++i) {
    state_ptr = sha256_multiple(&msgs[i]);
  ...
  printf("sha_avx_intel elapsed %0.2fsec i.e. %0.2f hashes/sec = 2^%0.3f hashes\n",
	 elapsed, NMSGS/elapsed,  log2(NMSGS/elapsed));
  return ctr;
}
...
int main(int argc, char* argv[])
{
 ...
 /*  vsha elapsed 3.53sec i.e. 4538645.35 hashes/sec = 2^22.114 hashes  */
  ...
  time_sha_ni();
  time_sha_avx(); /* intel implementation of sha256 */
